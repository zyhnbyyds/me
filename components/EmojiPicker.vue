...
<template>
  <div ref="pickerRef" relative flex-col-center select-none>
    ...
    <Transition name="fade">
      <div v-if="pickerVisible" class="top-100%" absolute left-0 z-100 mt5 bg-light-2 p-2 shadow-md>
        <div max-h-60 w-84 overflow-auto>
          <div v-for="item in emojiList" :key="item" cursor-pointer rounded-1.5 transition-all hover:bg-light7 class="h-10 w-10 flex-center inline-flex" @click="emits('select', hdImgUrl(item))">
