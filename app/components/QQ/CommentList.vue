...
function CommentName(props: { name: string; content: string }) {
  if (props.content.match(/@\{[^}]*\}/g)) {
    ...
    return (
      <span class='text-dark font-600 mr-1 dark:text-gray100'>
        <QQContentRender
          customClass='mr-0.5 text-dark font-600 dark:text-gray100 leading-normal mt0 text-3!'
          emojiSize='small'
          content={`${props.name}`}
        />
  ...
  return (
    <QQContentRender
      customClass='mr-1 text-dark font-600 dark:text-gray100 leading-normal mt0 text-3!'
      emojiSize='small'
      content={`${props.name}:`}
    />
  )
  ...
  return (
    <div class='text-3 flex'>
      <div class='flex flex-wrap gap-1 w-full items-center'>
        ...
        <QQContentRender
          emoji-size='small'
          custom-class='text-3! mt0! leading-normal! text-dark dark:text-light600!'
          content={transCon}
        />
      </div>
    </div>
